...
const ConfigEditor = {
    ...
    // 核心加载逻辑：获取硬件配置大小并完整读取
    async load(manager) {
        if (this.isBusy) return;
        this.isBusy = true;
        this.lockUI(true);
        manager.isBusy = true; // 独占 USB
            ...
            <div class="flex flex-col font-mono text-sm bg-white dark:bg-slate-900/40 rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800 shadow-xl">
                ${rows.map((line, i) => `
                    <div class="editor-line flex items-center border-b border-slate-100 dark:border-slate-800/50 transition-colors">
                        ...
                        <input type="text" class="flex-1 bg-transparent px-4 py-2.5 outline-none text-slate-700 dark:text-slate-200" value="${line.replace(/"/g, '&quot;')}" data-idx="${i}">
                    </div>
                `).join('')}
            </div>`;
    ...
    async sync() {
        ...
        try {
            ...
            this.lines.forEach((line, i) => {
                ...
                if (line.isDirty) {
                    const ok = line.current === newValue;
